...
FILE           *lstfile = nullptr;

// do_list returns true if listing is enabled.
static bool dolist(void) {
  int ok = lstfile != nullptr && pass > 0 && list_level > 0;
  return ok;
}

/* list_source saves a text line for later listing by list_flush */

void list_source(STREAM* str, char* cp) {
  if (dolist()) {
    int len = strcspn(cp, "\n");

    /* Save the line text away for later... */
    if (listline) free(listline);
    listline = static_cast<char*>(memcheck(malloc(len + 1)));
    memcpy(listline, cp, len);
    listline[len] = 0;

    if (!binline) {
      binline = static_cast<char*>(memcheck(malloc(sizeof(LSTFORMAT) + 16)));
    }

    snprintf(binline, sizeof(LSTFORMAT), "%*s%*d",
             static_cast<int>(sizeof(LSTFORMAT::flag)), "",
             static_cast<int>(sizeof(LSTFORMAT::line_number)), str->line);
  }
}
    ...
    unsigned addr)
{
    ...
    if (strlen(binline) >= col1) {
      ...
      snprintf(binline, sizeof(LSTFORMAT), "%*s%*d %6.6o",
               static_cast<int>(sizeof(LSTFORMAT::flag)), "",
               static_cast<int>(sizeof(LSTFORMAT::line_number)), str->line, addr);
      padto(binline, offsetof(LSTFORMAT, words));
    }
}
...
void list_word(STREAM* str, unsigned addr, unsigned value, int size,
  ...
  if (dolist()) {
    list_fit(str, addr);
    if (size == 1)
      snprintf(binline + strlen(binline), sizeof(LSTFORMAT) - strlen(binline),
               "   %3.3o%1.1s ", value & 0377, flags);
    else
      snprintf(binline + strlen(binline), sizeof(LSTFORMAT) - strlen(binline),
               "%6.6o%1.1s ", value & 0177777, flags);
  }
}
...
void report(STREAM* str, const char* fmt, ...) {
  ...
  if (!pass) return; /* Don't report now. */
