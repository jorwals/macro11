...
// This function is way way too large, because I just coded most of the
// operation code and pseudo-op handling right in line.

static std::optional<bool> assemble(STACK* stack, TEXT_RLD* tr) {
  ...
  if (line == nullptr) return std::nullopt; /* Return code for EOF. */
  ...
  if (suppressed) {
    ...
    if (op == nullptr) return true; /* Not found.  Don't care. */
    if (op->section->type != SECTION_PSEUDO) return true; /* Not a pseudo-op. */
    switch (op->value) {
      case P_IF:
      case P_IFDF:
      ...
      case P_IFF:
        if (suppressed == 1) { /* Can reduce suppression from 1 to 0. */
          if (!conds[last_cond].ok) suppressed = 0;
        }
        break;
      case P_IFT:
        if (suppressed == 1) { /* Can reduce suppression from 1 to 0. */
      ...
      case P_ENDC:
        ...
        if (suppressed == 0) pop_cond(last_cond - 1); /* Re-enabled. */
        break;
    }
    return true;
  }
  ...
  opcp = cp;
  if ((label = get_symbol(cp, &ncp, &local)) != nullptr) {
    ...
    if (*ncp == ':') { /* Colon, for symbol definition? */
      ...
      if (!enabl_lsb && !local) lsb++;

      cp = skipwhite(ncp);
      opcp = cp;
      label = get_symbol(cp, &ncp, nullptr); /* Now, get what follows */
    }
  }
...
reassemble:
  ...
  if (EOL(*cp)) return true; /* It's commentary.  All done. */
  ...
  if (label) { /* Something looks like a label. */
    ...
    if (*ncp == '=') {
        ...
        if (current_pc->section->flags & PSECT_REL) {
          ...
          if (!express_sym_offset(value, &sym, &offset)) {
            report(stack->top, "Illegal ORG\n");
                      ...
                      (SYMBOLFLAG_GLOBAL | SYMBOLFLAG_DEFINITION)) ==
                     SYMBOLFLAG_GLOBAL) {
      ...
      if (value->type == EX_LIT) {
        ...
        sym = add_sym(label, value->data.symbol->value, flags,
                      value->data.symbol->section, &symbol_st);
      } else {
        report(stack->top,
               "Complex expression cannot be assigned "
               "to a symbol\n");
        ...
        if (!pass) {
          ...
          sym = add_sym(label, 0, SYMBOLFLAG_UNDEFINED, &absolute_section,
                        &symbol_st);
        } else
          sym = nullptr;
      }

      if (sym != nullptr) list_value(stack->top, sym->value);
    ...
    if (op && op->stmtno < stmtno) {
      ...
      macstr = expandmacro(stack->top, reinterpret_cast<MACRO*>(op), ncp);
      ...
      switch (op->section->type) {
        case SECTION_PSEUDO:
          switch (op->value) {
            ...
            case P_NLIST:
            case P_PRINT:
              return true; /* Accepted, ignored.  (An obvious
            ...
            case P_IDENT: {
              ...
              endc[1] = '\n';
              endc[2] = 0;
              len = static_cast<int>(strcspn(cp, endc));
              if (len > 6) len = 6;

              if (ident)     /* An existing ident? */
                free(ident); /* Discard it. */

              ident = static_cast<char*>(memcheck(malloc(len + 1)));
              memcpy(ident, cp, len);
              ident[len] = 0;
              upcase(ident);
            ...
            case P_NARG: {
              ...
              for (str = stack->top;
                   str != nullptr && str->vtbl != &macro_stream_vtbl;
                   str = str->next);
              ...
              mstr = reinterpret_cast<MACRO_STREAM*>(str);

              add_sym(label, mstr->nargs, SYMBOLFLAG_DEFINITION | local,
            ...
            case P_NCHR: {
              ...
              add_sym(label, strlen(string), SYMBOLFLAG_DEFINITION | local,
                      &absolute_section, &symbol_st);
              free(label);
              free(string);
              return true;
            ...
            case P_NTYPE: {
              ...
              add_sym(label, mode.type, SYMBOLFLAG_DEFINITION | local,
                      &absolute_section, &symbol_st);
              free_addr_mode(&mode);
              free(label);
            ...
            case P_REM: {
              ...
              for (;;) {
                cp += strcspn(cp, quote);
                if (*cp == quote[0]) break; /* Found closing quote */
                cp = stack_gets(stack);     /* Read next input line */
                if (cp == nullptr) break;      /* EOF */
              }
            }
              return true;
            ...
            case P_MCALL: {
              ...
              for (;;) {
                ...
                macbuf = nullptr;
                for (i = 0; i < nr_mlbs; i++)
                  if ((macbuf = mlb_entry(mlbs[i], label)) != nullptr) break;
                if (macbuf != nullptr) {
                  ...
                  strncat(macfile, ".MAC",
                          sizeof(macfile) - strlen(macfile) - 1);
                ...
                if (macstr != nullptr) {
                  for (;;) {
                    ...
                    if (maccp == nullptr) break;
                    mlabel = get_symbol(maccp, &maccp, nullptr);
                    if (mlabel == nullptr) continue;
                    op = lookup_sym(mlabel, &system_st);
                    free(mlabel);
                    if (op == nullptr) continue;
                    if (op->value == P_MACRO) break;
                  }
            ...
            case P_REPT: {
              ...
              if (reptstr) stack_push(stack, reptstr);
              return reptstr != nullptr;
            }
            ...
            case P_TITLE:
              ...
              module_name = get_symbol(cp, &cp, nullptr);
              return true;
            ...
            case P_END:
              ...
              cp = skipwhite(cp);
              if (!EOL(*cp)) {
                if (xfer_address) free_tree(xfer_address);
            ...
            case P_IF: {
              ...
              label = get_symbol(cp, &cp, nullptr); /* Get condition */
              cp = skipdelim(cp);
              ...
              if (strcmp(label, "DF") == 0) {
                ...
                if (!EOL(*cp))
                  ...
                  thing = static_cast<char*>(memcheck(strdup("")));
                ok = (*thing == 0);
                free(thing);
                ...
                if (!EOL(*cp))
                  ...
                  thing2 = static_cast<char*>(memcheck(strdup("")));
                ...
                if (value->type != EX_LIT) {
                  ...
                  if (strcmp(label, "EQ") == 0 || strcmp(label, "Z") == 0) {
                    ...
                    // TODO(jorgen): I added the following error! Correct?
                    report(stack->top, "Bad .IF condition\n");
                    ok = false;
                    word = 0;
                  }
            ...
            case P_PSECT: {
              ...
              if (label == nullptr)
                label =
                    static_cast<char*>(memcheck(strdup(""))); /* Allow blank */
            ...
            case P_GLOBL: {
              ...
              while (!EOL(*cp)) {
                ...
                label = get_symbol(cp, &ncp, nullptr);
                if (label == nullptr) {
                  report(stack->top,
                         "Illegal .GLOBL/.WEAK "
                         "syntax\n");
            ...
            case P_RAD50:
              ...
              while (!EOL(*cp)) {
                ...
                radstr = static_cast<char*>(memcheck(malloc(len + 1)));
                ...
                for (radp = radstr; *radp;) {
                  ...
                  rad = rad50(radp, const_cast<const char**>(&radp));
          ...
          switch (op->flags & OC_MASK) {
            ...
            case OC_BR:
              /* branches */ {
                ...
                if (!check_branch(stack, offset, -256, 255)) offset = 0;
            ...
            case OC_SOB: {
              ...
              if (!check_branch(stack, offset, 0, 126)) offset = 0;
...
int assemble_stack(STACK* stack, TEXT_RLD* tr) {
  std::optional<bool> res;
  int errcount = 0;

  while ((res = assemble(stack, tr)).has_value()) {
    list_flush();
    if (!*res) errcount++;  // Count an error
  }
